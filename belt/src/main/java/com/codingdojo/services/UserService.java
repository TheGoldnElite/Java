...
import com.codingdojo.respositories.UserRepository;
...
public class UserService {
	...
	public User register(User newUser,BindingResult result) {
		...
		//2.check password is != confirm
		if(!newUser.getPassword().equals(newUser.getConfirm())) {
			result.rejectValue("confirm", "matches","The password and confirm do not match");
		}
	...
	public User login(LoginUser newLogin, BindingResult result) {
		...
		//2.if email Not present,reject and return
		if(!potentialUser.isPresent()) {
			result.rejectValue("email","unique","The email already exists");
			return null;
		}
